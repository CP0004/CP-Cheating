...
void UpdateValue(Response &response, Request &request)
{
    uintptr_t uWorld = getPtr(getPtr(getPtr(getRealOffset(Offsets::GWorld)) + 0x38) + 0x58);
    uintptr_t uLevel = getPtr(uWorld + Offsets::PersistentLevel);
    uintptr_t PlayerController = getPtr(getPtr(getPtr(uWorld + 0x24) + 0x60) + 0x20);
    uintptr_t PlayerCameraManager = getPtr(PlayerController + Offsets::PlayerCameraManager);

    uintptr_t LocalPlayer;
    uintptr_t gname = getPtr(getPtr(libbase + Offsets::GNames) + 0x80);
    ...
    uintptr_t entityPtr = getActorsArray(uLevel, Offsets::ULevelToAActors, 0x334);
